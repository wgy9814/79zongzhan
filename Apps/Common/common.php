 ...
 * @return boolean
 */
function verify_groupid($type) {
    $html = '';
    switch($type) {
        case 9:
            $html .= '<span class="nc-grade-mini">v普</span>';
            break;
        case 10:
            $html .= '<span class="nc-grade-middle">v高</span>';
            break;
        case 11:
            $html .= '<span class="nc-grade-top">v钻</span>';
            break;
    }
    return $html;
}
 ...
 * @return boolean
 */
function change_groupid() {
    $now = time();
    $where = [];
    $where['status'] = 1;
    $where['expire_time'] = array('lt',$now);
    $user_level = M('User_level')->field('userid')->where($where)->select();//查找审核通过且过期的申请

    if($user_level) {
        $data['groupid'] = 6;
        foreach ($user_level as $val) {
            $list[] = $val['userid'];
            M('User')->where("id=".$val['userid'])->save($data);//改变会员用户组为6
        }

        $where = [];
        $where = array('id' => array('in', $list));
        $school_list = M('User')->field('schoolid')->where($where)->select();
//        echo M('User')->getLastSql();
//        print_r($user);
        foreach ($school_list as $v){
            $school = M('School');
            $school->where("id=".$v['schoolid'])->save($data);
            $kecheng = M('Kecheng');
            $kecheng->where("school_id=".$v['schoolid'])->save($data);
        }
        ...
        $data = [];
        $data['status'] = 0;
        M('User_level')->where($where)->save($data);//将审核通过且过期的申请变为未审核
...
function filterArr($array)
{
    ...
    if (!is_array($array)) return filterRequest($array);
    foreach ($array as $k => $val) $array[$k] = filterArr($val);
    return $array;
}
// sql注入过滤
function filterRequest($str)
{
