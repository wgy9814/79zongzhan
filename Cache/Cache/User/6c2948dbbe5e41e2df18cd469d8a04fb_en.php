<?php if (!defined('THINK_PATH')) exit();?><!DOCTYPE html><html><head lang="en"><meta name="description" content=""><meta name="keywords" content=""><meta charset="utf-8"><meta name="renderer" content="webkit"><!--360 极速模式--><link rel="shortcut icon" href="/Apps/Tpl/Home/Default/Public/images/favicon.ico" /><meta http-equiv="Content-Type" content="text/html; charset=utf-8" /><meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1"><title>账户管理-79招生网</title><link href="../Public/css/common_header_footer.css" rel="stylesheet" type="text/css" /><link href="../Public/css/common_base.css" rel="stylesheet" type="text/css" /><link href="../Public/css/member.css" rel="stylesheet" type="text/css" /><script type="text/javascript" src="/Public/Js/jquery-2.1.1.min.js"></script><style type="text/css">
    ...
    </style></head><body><div class="header"><!--logo搜索--><div class="top_nav"><div class="w1200 auto clearfix" id="user_head"><div class="top_logo"><a class="l" href="/"><img src="<?php echo ($logo); ?>" /></a><div class="top_qie">
        ...
        <?php if($user_mark) : echo ($user_mark); endif;?></div><div class="user-info"><dl class="user-info-dl"><?php if($user_mark) : ?><dd class="">V生效时间
                ...
                <a href="javascript:;"><?php echo (todate($vo["createtime"],'Y-m-d H:i:s')); ?></a></dd></dl></div></div><div class="side-manu"><dl><dt>账户管理</dt><dd><a class="active" href="/User/Index">个人资料</a></dd><dd><?php if($vo['user']) : ?><a class="" id="fabu" href="javascript:;">我的课程及发布新课程</a><?php else :?><a class="" href="/User/Index/mykecheng2?sid=<?php echo ($vo['id']); ?>">我的课程</a><?php endif;?></dd><?php if($vo['user']) : else :?><dd><a href="/User/Index/mycoupon?sid=<?php echo ($vo['id']); ?>">优惠卷</a></dd><?php endif;?><dd><a href="/User/Index/news">我的消息</a></dd><?php if($vo['groupid'] != '3') : ?><dd><a href="/User/Index/vip">自主招生服务V</a></dd><dd><a href="javascript:;" id="treaty">代理招生服务D</a></dd><?php endif;?></dl></div></div><div id="tip" style="display: none;"><div id="new_kecheng_tip"><p class="MsoNormal" style="text-align:left;white-space:normal;text-indent:2em;">	免费发布课程及自主招生注意事项
					...
					</p></td></tr></tbody></table></p></div></div><script type="text/javascript" src="/Public/Js/layer.js"></script><script type="text/javascript" src="/Public/Js/common.js"></script></div><div class="main-right"><form enctype="multipart/form-data" class="tab-pane active" id="fabu" method="post" action="<?php echo URL('User-Index/addvip');?>" name="theForm" onsubmit="return verify();"><table class="pure-table"><?php  $_result=M()->query("SELECT * FROM mqu_vip WHERE status =1"); if ($_result): $rvx=0;foreach($_result as $key=>$table):++$rvx;$mod = ($rvx % 2 );?><caption><?php echo ($table["caption"]); ?></caption><thead><tr class="pure-table-odd"><th><?php echo ($table["title_1"]); ?></th><th><?php echo ($table["title_2"]); ?></th><th><?php echo ($table["title_3"]); ?></th><th><?php echo ($table["title_4"]); ?></th></tr></thead><tr class="pure-table-odd"><td><?php echo ($table["content_one_1"]); ?></td><td><?php echo ($table["content_one_2"]); ?></td><td><?php echo ($table["content_one_3"]); ?></td><td><?php  $_result=M()->query("SELECT * FROM mqu_role WHERE status =1 and name='vip普通会员'"); if ($_result): $rvx=0;foreach($_result as $key=>$user):++$rvx;$mod = ($rvx % 2 );?><input type="radio" name="type"  id="mini" value="<?php echo ($user["id"]); ?>" onclick="getValue(this)"><?php endforeach; endif;?><label for="mini" class="for_mini"><div id="mini_label"></div></label></td></tr><tr class="pure-table-odd"><td><?php echo ($table["content_two_1"]); ?></td><td><?php echo ($table["content_two_2"]); ?></td><td><?php echo ($table["content_two_3"]); ?></td><td><?php  $_result=M()->query("SELECT * FROM mqu_role WHERE status =1 and name='vip高级会员'"); if ($_result): $rvx=0;foreach($_result as $key=>$user):++$rvx;$mod = ($rvx % 2 );?><input type="radio" name="type"  id="middle" value="<?php echo ($user["id"]); ?>" onclick="getValue(this)"><?php endforeach; endif;?><label for="middle" class="for_middle"><div id="middle_label"></div></label></td></tr><tr class="pure-table-odd"><td><?php echo ($table["content_three_1"]); ?></td><td><?php echo ($table["content_three_2"]); ?></td><td><?php echo ($table["content_three_3"]); ?></td><td><?php  $_result=M()->query("SELECT * FROM mqu_role WHERE status =1 and name='vip钻石会员'"); if ($_result): $rvx=0;foreach($_result as $key=>$user):++$rvx;$mod = ($rvx % 2 );?><input type="radio" name="type"  id="top" value="<?php echo ($user["id"]); ?>" onclick="getValue(this)"><?php endforeach; endif;?><label for="top" class="for_top"><div id="top_label"></div></label></td></tr><tr class="pure-table-odd" ><td colspan="2" ><?php echo ($table["content_four_1"]); ?></td><td><?php echo ($table["content_four_2"]); ?></td><td><?php  $_result=M()->query("SELECT * FROM mqu_role WHERE status =1 and name='定制服务'"); if ($_result): $rvx=0;foreach($_result as $key=>$user):++$rvx;$mod = ($rvx % 2 );?><input type="radio" name="type"  id="define" value="<?php echo ($user["id"]); ?>" onclick="getValue(this)"><?php endforeach; endif;?></td></tr><tr class="pure-table-odd" ><td colspan="2"><?php echo ($table["content_five_1"]); ?></td><td colspan="2"><?php echo ($table["content_five_2"]); ?></td></tr><?php endforeach; endif;?></table><button type="submit" class="vip-btn">提交</button></form></div></div></div></div></div><link rel="stylesheet" href="../Public/css/ziliao.css" /><!--底部--><div class="footer mt20 clear"><div class="site_map"><div class="site_map_con site_map_con w1100 auto"><ul class="inline_box"><li class="inline_any" style="background:#fff;padding:8px;width: 112px;text-align: center;margin-right: 111px;"><?php $r = M('Block')->where(" 1  and lang=2 and pos='weima' ")->find(); if ($r):?><p><img src="<?php echo ($r["image1"]); ?>" /></p><?php endif;?><p class="" style="color:#666;">让学习变得更容易</p></li><?php $k=0;foreach($Categorys as $key=>$r):if(1=="" && $r['isfootermenu']==0){ continue; }if( $r['ismenu']==1 && intval(39)==$r["parentid"] ) :++$k;?><li class="inline_any"><h3 class="site_map_title"><?php echo ($r["catname"]); ?></h3><?php if($r[child]) : $kd=0;foreach($Categorys as $key=>$rd):if(1=="" && $rd['isfootermenu']==0){ continue; }if( $rd['ismenu']==1 && intval($r[id])==$rd["parentid"] ) :++$kd;?><p><a rel="nofollow" target="_bank" href="<?php echo ($rd["url"]); ?>"><?php echo ($rd["catname"]); ?></a></p><?php endif; endforeach; endif;?></li><?php endif; endforeach;?></ul></div></div><div class=" clear"></div><div class="lxwm tc mt10 w1100 auto"><ul><li style="font-size:14px;"><p><span style="color:#FFFFFF;font-size:14px;text-align:center;white-space:normal;background-color:#40B477;font-family:微软雅黑;">79招生网 版权所有 </span><span white-space:normal;background-color:#ffffff;"="" style="text-decoration: none; color: rgb(255, 255, 255); padding: 1px 0px; outline: 0px; font-size: 14px; text-align: center; white-space: normal; background-color: rgb(64, 180, 119); font-family: 微软雅黑;">备案/许可证编号为：<a rel="nofollow" href="http://beian.miit.gov.cn" target="_blank">粤ICP备</a></span><a rel="nofollow" href="http://beian.miit.gov.cn" target="_blank"><span t="7" data="17158495" white-space:normal;background-color:#ffffff;border-bottom:1px="" dashed="" #cccccc;z-index:1;position:static;"="" style="text-decoration: none; color: rgb(255, 255, 255); padding: 1px 0px; outline: 0px; font-size: 14px; text-align: center; white-space: normal; background-color: rgb(64, 180, 119); font-family: 微软雅黑;">17158495</span><span white-space:normal;background-color:#ffffff;"="" style="text-decoration: none; color: rgb(255, 255, 255); padding: 1px 0px; outline: 0px; font-size: 14px; text-align: center; white-space: normal; background-color: rgb(64, 180, 119); font-family: 微软雅黑;">号</span></a></p><p>	Copyright &copy; 2017 All rights reserved by 79招生网
</p> | Designed bymqu.cn</li></ul></div></div><script>
    // function verify() {
    //     document.forms['theForm'].elements['level'].checked
    //     var ifCheck=document.getElementsByName('gender')[0].checked ||document.getElementsByName('gender')[1].checked?true:false; if(!ifCheck){ alert("您没有选择性别。");
    //
    // }
        function verify(){
            var theCheckboxInputs=document.getElementsByName('theForm');
    ...
    function getValue(obj){
        var val = obj.value;
        if(val == "mini"){
            document.getElementById("mini_label").style.backgroundColor="red";
            document.getElementById("middle_label").style.backgroundColor="#FD88A3";
            document.getElementById("top_label").style.backgroundColor="#FD88A3";
        }else if(val == "middle"){
    	...
    	if(msg){
    		if(msg.data.length>0){
	        	...
	        	for (var i =0;i<message.length;i++) {
					...
					list_html+='<div class="pb"><div class="con c6">'+message[i].content+'</div><a href="javascript:;"></a></div></div></div></li>'
	        	}
	        	if(msg.count>0){
	        	    if(type ==1){
